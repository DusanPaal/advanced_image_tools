    ...
    if (e.target.files) {
      ...
      filesArray.forEach((file) => {
        if (file.type.startsWith('image/')) {
          ...
          img.onerror = () => {
            setFeedback('Error while uploading the image.');
            hideLoaderAfterDone(filesLength);
          };
      ...
      if (wrongFilesNames.length > 0) {
        const errorLabel =
          wrongFilesNames.length > 1 ? 'are not images' : 'is not image';
        setFeedback(
          `${wrongFilesNames.join(', ')} ${errorLabel} 'to be uploaded'.`,
        );
      }
    }
  ...
  const menuItems = useMemo(
    () => [
      isUploadEnabled && {
        ...
        label: isLoading ? 'Importing...' : 'Upload image',
        icon: UploadOutline,
